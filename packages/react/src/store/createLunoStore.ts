...
export const useLunoStore = create<LunoState>((set, get) => ({
    ...
    _setConfig: (newConfig) => {
      ...
      const initialChainId = newConfig.chains[0]?.genesisHash;
      const initialChain = initialChainId
        ? newConfig.chains.find(c => c.genesisHash === initialChainId)
        : undefined;
    ...
    connect: async (connectorId, targetChainId) => {
      ...
      if (!config) {
        const err = new Error('[LunoStore] LunoConfig has not been initialized. Cannot connect.');
        set({error: err, status: ConnectionStatus.Disconnected});
      ...
      if (!connector) {
        const err = new Error(`[LunoStore] Connector with ID "${connectorId}" not found in LunoConfig.`);
        set({error: err, status: ConnectionStatus.Disconnected});
        console.error(err.message);
        return;
      }
      ...
      try {
        ...
        set({
          ...
          rawAccounts: accountsFromWallet,
        ...
        console.error(`[LunoStore] Error connecting with ${connector.name}:`, err);
        cleanupActiveConnectorListeners();
        set({
          status: ConnectionStatus.Disconnected,
          error: err,
    ...
    disconnect: async () => {
      ...
      set({ status: ConnectionStatus.Disconnecting, error: null });
      try {
        ...
        set({ status: ConnectionStatus.Disconnected, activeConnector: undefined, rawAccounts: [], error: null })
        ...
        console.error(`[LunoStore] Error disconnecting from ${activeConnector.name}:`, err);
        set({status: ConnectionStatus.Connected, error: err}); // Revert status if disconnect call failed
      }
    },
    ...
    switchChain: async (newChainId: string) => {
      ...
      if (!config) {
        const err = new Error('[LunoStore] LunoConfig has not been initialized. Cannot switch chain.');
        set({error: err});
        console.error(err.message);
        return;
      }
      ...
      if (!newChain) {
        const err = new Error(`[LunoStore] Chain with ID "${newChainId}" not found in LunoConfig.`);
        set({error: err});
        console.error(err.message);
        return;
      }
      ...
      if (config.storage) {
        try {
          ...
          console.error('[LunoStore] Failed to persist new chainId to storage:', e);
        }
      }
    }
  })
