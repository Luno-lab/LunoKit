...
import { isValidAddress, convertAddress, isSameAddress, getPublicKey, mapInjectedAccounts } from './address'
...
describe('address utils', () => {
  ...
  describe('convertAddress', () => {
    ...
    it('should throw error for invalid address', () => {
      expect(() => convertAddress('invalid', 0))
        .toThrow('Failed to convert address')
      expect(() => convertAddress('0x4838b106fce9647bdf1e7877bf73ce8b0bad5f97', 0))
        .toThrow('Failed to convert address')
      expect(() => convertAddress('', 0))
        .toThrow('Failed to convert address')
    })
    ...
    it('should throw error for invalid SS58 format', () => {
      ...
      expect(() => convertAddress(validAddress, -1))
        .toThrow('Failed to convert address')
      expect(() => convertAddress(validAddress, 99999))
        .toThrow('Failed to convert address')
    })
  })
  ...
  describe('getPublicKey', () => {
    ...
    describe('error handling', () => {
      it('should throw error for invalid address', () => {
        ...
        expect(() => getPublicKey('this-is-definitely-invalid')).toThrow('Failed to get public key')
      })

      it('should handle ethereum addresses appropriately', () => {
        const ethAddress = '0x4838b106fce9647bdf1e7877bf73ce8b0bad5f97'
        expect(() => getPublicKey(ethAddress)).toThrow('Invalid public key length: expected 32 bytes')
      })

      it('should throw error for malformed addresses', () => {
        expect(() => getPublicKey('1FRMM8PEiWXYax7rpS6X4XZX1aAAxSWx1CrKTyrVYhV24fg@'))
          .toThrow('Failed to get public key')

        expect(() => getPublicKey('1FRMM8PEiWXYax7rpS6X4XZX1aAAxSWx1CrKTyrVYhV24fg123'))
          .toThrow('Failed to get public key')
  ...
  describe('mapInjectedAccounts', () => {
    ...
    describe('edge cases', () => {
      it('should return empty array for null input', () => {
        const result = mapInjectedAccounts(null as any, 'test-source')
        expect(result).toEqual([])
      })

      it('should return empty array for undefined input', () => {
        const result = mapInjectedAccounts(undefined as any, 'test-source')
        expect(result).toEqual([])
      })

      it('should return empty array for empty array input', () => {
        const result = mapInjectedAccounts([], 'test-source')
        expect(result).toEqual([])
      })
    })
