...
import { ChainType, type HexString, type SubstrateAccount } from '../../types';
...
/**
 * map injected accounts to internal SubstrateAccount type
...
export function mapInjectedAccounts(
  ...
  try {
    return injectedAccounts.map((acc: InjectedAccount) => {
      ...
      const mappedAccount: SubstrateAccount = {
        address: acc.address,
        name: acc.name,
