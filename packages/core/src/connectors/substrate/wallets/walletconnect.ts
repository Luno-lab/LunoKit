...
export class WalletConnectConnector extends SubstrateConnector {
  ...
  private connectedChains?: Optional<SubstrateChain[] | HexString[]>;
  ...
  constructor(options: WalletConnectConnectorOptions) {
    ...
    this.icon = options.icon || walletconnectSubstrateWallet;
  ...
  private convertChainIdToCaipId(chains: SubstrateChain[] | HexString[]): `polkadot:${string}`[] {
    return chains.map((chain) => {
      const capid: HexString = (chain as SubstrateChain).genesisHash || chain;
  ...
  public async connect({
    appName,
    chains,
  }: SubstrateConnectOptions): Promise<Array<SubstrateAccount> | undefined> {
    if (!this.projectId) {
      throw new Error(
  ...
  private getAccountsFromSession(chains: SubstrateChain[] | HexString[]): SubstrateAccount[] {
    const targetChain = [(chains[0] as SubstrateChain).genesisHash || chains[0]] as
      | HexString[]
      | SubstrateChain[];
    ...
    const result: SubstrateAccount[] = rawAccounts!.map((address: string) => ({
      address,
  ...
  public async getSigner(): Promise<SubstrateSigner | undefined> {
